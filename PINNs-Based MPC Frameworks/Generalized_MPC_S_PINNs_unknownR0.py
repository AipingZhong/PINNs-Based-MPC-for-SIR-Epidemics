...
tf = 85.0  # Total simulation time

# MPC parameter settings
N_pri = 14  # Prediction horizon length
alpha1, alpha2 = 1e3, 1.0  # Cost function weights
I_max = 0.13  # Maximum allowable infected proportion
u_max = 0.3  # Maximum control input
Ts = 5  # Sampling period
start_control = 45  # Control start time
end_control = 79  # Control end time
end_control_int = int(end_control)
S_target = 1 / 2.5  # Target susceptible proportion (herd immunity threshold)

# Training and optimizer settings
loss_err = 'mse'
optimizer = 'adam'
adaptive_NTK = {'method': 'NTK', 'freq': 100}
Nc = 6000  # Number of collocation points

# Simplified training strategy: more epochs for the first training, fewer epochs for subsequent training with weight reuse
epochs_ode_initial = 5000   # Number of epochs for the initial ODE training
epochs_data_initial = 2500  # Number of epochs for the initial data regression training
epochs_ode_reuse = 1000     # Number of epochs for ODE training with weight reuse
epochs_data_reuse = 500     # Number of epochs for data regression training with weight reuse

# Number of experiment runs
num_runs = 3

# Simplified weight storage variables
prev_weights_by_run = {}
...
gamma_est_mean = np.zeros(tf_int + 1)
...
gamma_est_std = np.zeros(tf_int + 1)
...
for k in range(start_control, int(tf)):
    ...
    if ((k - start_control) % Ts == 0 or k == start_control) and k <= end_control:
        ...
        for run in range(num_runs):
            ...
            # Directly use the previous weights (if available)
            weight_reused = False
            ...
            print(f"Time step {k}, Run {run + 1}: data regression completed, final epoch loss: {loss_data:.5e}")
            ...
            Beta = sn.Parameter(0.5, name='Beta', inputs=t, non_neg=True)

            # Try loading the weights of the physics-informed network
            ...
            # Train the model
            log_params = {'parameters': [Beta, Gamma], 'freq': 1}
            time1_ode = time.time()
            history_ode = pinn_ode.train(t_train_ode,
                                         loss_train_ode,
            ...
            print(
                f"[Time Step {k} | Run {run + 1}] Estimated Values: beta = {beta_est_val:.10f}, gamma = {gamma_est_val:.10f}, S = {S_est_full[k]:.10f}, I = {I_est_full[k]:.10f}, R = {R_est_full[k]:.10f}")
            print(
                f"[Time Step {k} | Run {run + 1}] True Values:      beta = {beta:.10f}, gamma = {gamma:.10f}, S = {S_true[k]:.10f}, I = {I_true[k]:.10f}, R = {R_true[k]:.10f}")
            ...
            gamma_est_runs[t_est_steps, run] = gamma_est_val
        ...
        gamma_values = gamma_est_runs[k, :]
        if num_runs >= 3:
            max_index = np.argmax(gamma_values)
            min_index = np.argmin(gamma_values)
            indices_to_delete = [max_index] if max_index == min_index else [max_index, min_index]
            gamma_filtered = np.delete(gamma_values, indices_to_delete)
        else:
            gamma_filtered = gamma_values
        gamma_est_mean[k] = np.mean(gamma_filtered)
        gamma_est_std[k] = np.std(gamma_filtered)
        ...
        gamma_est_val = gamma_est_mean[k]
        ...
        for i in range(N_pri):
            ...
            gamma_u_est = gamma_est_val + current_u
...
if True:
    ...
    for run in range(num_runs):
        ...
        time_data_used = time2_data - time1_data
        loss_data = history_data.history['loss'][-1]
        print(f"Time step {int(tf)} data regression completed, final epoch loss: {loss_data:.5e}")
        print(f"Data regression training time: {time_data_used:.2f} seconds")
        print(f"Epochs used: {epochs_data_current}, Weight reused: {weight_reused}")
        ...
        # Generate collocation points
        t_train_ode = np.random.uniform(np.log1p(0 / tf), np.log1p(1), Nc).reshape(-1, 1)
        t_train_ode = np.exp(t_train_ode) - 1.
        ...
        print(
            f"Time step {tf_int}, Run {run + 1} Estimated Values | beta: {beta_est_val:.10f}, gamma: {gamma_est_val:.10f}, S: {S_est_full[tf_int]:.10f}, I: {I_est_full[tf_int]:.10f}, R: {R_est_full[tf_int]:.10f}")
        print(
            f"Time step {tf_int}, Run {run + 1} True Values      | beta: {beta:.10f}, gamma: {gamma:.10f}, S: {S_true[tf_int]:.10f}, I: {I_true[tf_int]:.10f}, R: {R_true[tf_int]:.10f}")
        ...
        gamma_est_runs[last_training_time + 1:tf_int + 1, run] = gamma_est_val
        ...
        loss_data_runs[tf_int, run] = loss_ode
...
gamma_est_mean, gamma_est_std = compute_mean_std_without_extremes(gamma_est_runs)
...
loss_data_mean, _ = compute_mean_std_without_extremes(loss_data_runs)
...
if use_weight_reuse:
    if kappa == 1:
        output_dir = 'Plot/Gen_MPC_S_PINNs_kappa1_beta_gamma_noR0_simple_reuse'
    elif kappa == 0.001:
        output_dir = 'Plot/Gen_MPC_S_PINNs_kappa0.001_beta_gamma_noR0_simple_reuse'
else:
    if kappa == 1:
        output_dir = 'Plot/Gen_MPC_S_PINNs_kappa1_beta_gamma_noR0_no_reuse'
    elif kappa == 0.001:
        output_dir = 'Plot/Gen_MPC_S_PINNs_kappa0.001_beta_gamma_noR0_no_reuse'
