...
Nc = 1  # Number of collocation points 6000

# Training strategy with weight reuse
epochs_ode_initial = 6000  # Number of epochs for the initial training 6000
epochs_ode_reuse = 1200    # Number of epochs for training after reusing weights 1200
...
for k in range(start_control, int(tf)):
    ...
    if ((k - start_control) % Ts == 0 or k == start_control) and k <= end_control:
        ...
        for run in range(num_runs):
            ...
            # Train the model
            log_params = {'parameters': [Gamma, Beta], 'freq': 1}
            time1_ode = time.time()
...
if True:
    ...
    for run in range(num_runs):
        ...
        print(f"Time step {tf_int}, Run {run + 1} Estimated Values | beta: {beta_est_val:.10f}, gamma: {gamma_est_val:.10f}, S: {S_est_full[tf_int]:.10f}, I: {I_est_full[tf_int]:.10f}, R: {R_est_full[tf_int]:.10f}")
        print(f"Time step {tf_int}, Run {run + 1} True Values      | beta: {beta:.10f}, gamma: {gamma:.10f}, S: {S_true[tf_int]:.10f}, I: {I_true[tf_int]:.10f}, R: {R_true[tf_int]:.10f}")
...
if use_weight_reuse:
    if kappa == 1:
        output_dir = 'Plot/Gen_MPC_LS_PINNs_kappa1_beta_gamma_noR0_reuse'
    elif kappa == 0.001:
        output_dir = 'Plot/Gen_MPC_LS_PINNs_kappa0.001_beta_gamma_noR0_reuse'
else:
    if kappa == 1:
        output_dir = 'Plot/Gen_MPC_LS_PINNs_kappa1_beta_gamma_noR0_no_reuse'
    elif kappa == 0.001:
        output_dir = 'Plot/Gen_MPC_LS_PINNs_kappa0.001_beta_gamma_noR0_no_reuse'
